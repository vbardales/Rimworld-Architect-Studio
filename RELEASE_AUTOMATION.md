...
| `release.yml` | manual only | Defaults to a credential-free semantic-release dry run. `publish` pauses at `steam-production`; only after approval can it create the GitHub release/tag and update the existing Workshop item. |
...
1. Run `release.yml` in `dry-run` mode. It rebuilds `Mod/`, runs `Tests/Validate-Mod.ps1`, and only
   calculates the next semantic version and notes. It cannot create a tag, GitHub release, or Steam
   update because `STEAM_PUBLISH` and Steam credentials are absent.
