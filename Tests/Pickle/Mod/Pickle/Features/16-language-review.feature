...
Feature: the interface in the language the game runs in
  ...
  # The names read as English with accented letters on purpose: a name in the other language would look like an
  # untranslated string in a review capture, whichever language the pass runs in.
  Scenario: a very long accented name in both editors
    When I close all dialogs
    And I open the "Architect" tab
    And I click the Architect Studio button keyed "ArchitectStudio.ArchitectButton"
    Then window "Dialog_DropdownGroups" is open
    When I create the group "Café-side carpentry shed for chairs, crèche cots, tapestries from Zürich, jalapeño stores and every other bulky thing in the colony"
    And I select the group "Café-side carpentry shed for chairs, crèche cots, tapestries from Zürich, jalapeño stores and every other bulky thing in the colony" in the editor
    And I wait 30 ticks
    And Nelim's Pickle Tools: screenshot mode is enabled around the open windows
    ...
    And I close all dialogs
    And I open the category editor
    And I create the category "Crème brûlée pantry, smörgåsbord shelves and every long-keeping provision the colony stores"
    And I wait 30 ticks
    Then window "Dialog_Categories" is open
    When Nelim's Pickle Tools: screenshot mode is enabled around the open windows
