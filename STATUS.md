...
remaining:
  ...
  - verified: on branch `migrate-manual` (not merged, not pushed) the release workflow is `.github/workflows/publish-tag.yml`, generated by `Rimworld-Release-Admin/scripts/generate-publish-workflow.sh` with `.github/publish.config.json`; `release.yml`, `release-steam-plugin.mjs`, `release.config.mjs` and `package.json` are removed. Until that branch is merged, `main` still holds `release.yml` (brought to the template by PR 4, `b3776cc`, merged 2026-09-25 17:20 UTC). Neither has run on GitHub yet: the first dry-run is CI/CD's, on the exact final SHA. A 1.0.6 needs a dated `## [1.0.6]` in `CHANGELOG.md` and a `### 1.0.6` fenced block in `PUBLICATION.md` (first line carrying the version), or the dry-run stops on purpose
