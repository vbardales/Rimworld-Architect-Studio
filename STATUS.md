...
workshop:     3792784018
remaining:
  - unverified: counted phrases were rewritten as families of keys (`.Zero`, `.One`, `.Many`) on 2026-09-25 to satisfy the plural rule of TRANSLATIONS.md (`OverrideCount`, `HiddenCount`, `ConfirmDissolve`, `MoreResults`; `CountedText.cs`, four call sites in `Dialog_DropdownGroups.cs`); the mod validator (995 checks) and the behavior tests (34) pass, the game has not shown the new texts yet, hence `localization`, `translation_en` and `translation_fr` stay `partial` until a capture of each language is read
