...
workshop:     3792784018
remaining:
  - unverified: counted phrases were rewritten as families of keys (`.Zero`, `.One`, `.Many`) on 2026-09-25 for the plural rule of TRANSLATIONS.md (`OverrideCount`, `HiddenCount`, `ConfirmDissolve`, `MoreResults`; `CountedText.cs`, four call sites, validator now requires every form and refuses a `(s)` suffix; 995 checks and 34 behavior checks pass). Seen in the game on `4e86e04`: the footer of the group editor in the `.Zero` form, in English ("0 buildings moved from their original settings.") and in French ("0 bâtiment déplacé par rapport aux réglages d'origine."). Not seen: the `.One` and `.Many` forms, `HiddenCount`, `ConfirmDissolve` and `MoreResults`; hence `localization`, `translation_en` and `translation_fr` stay `partial`
  ...
  - verified: `.github/workflows/release.yml` was brought to the current template by PR 4 (`b3776cc`, merged 2026-09-25 17:20 UTC): `ref`/`version`/`mode` inputs, full-SHA guard, documented release (version from the input, notes from `CHANGELOG.md` and `PUBLICATION.md`), Steam description read from `PUBLICATION.md`. Not run on GitHub yet: its first dry-run is CI/CD's, on the exact final SHA. A 1.0.5 needs a dated `## [1.0.5]` in `CHANGELOG.md` and a `### 1.0.5` fenced block in `PUBLICATION.md`, or the dry-run stops on purpose
...
updated:      2026-09-25, 1.0.5 published through the pipeline (tag v1.0.5 on ccad168, GitHub release, Workshop change note and description read); stage stays done
---
...
| all passes, `68fe26f` | reset 3/3; very long accented name 1/1 in English and in French; full French reviews pass 51 scenarios, 42 passed, 0 failed, 9 skipped (the conditional ones of other passes: Architect Icons, studio, RIMMSQOL, restart), `exitReason: passed` |
